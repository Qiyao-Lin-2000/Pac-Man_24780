...
int main()
{
    ...
    // Fake player + ghosts + map for Play screen
    ...
    auto state = GameScreenState::Play;
    // Main loop: render Play screen; ESC to exit
    while(FsCheckWindowOpen()){
        ...
        if(FSKEY_P==key){
            state = state==GameScreenState::Pause ? GameScreenState::Play : GameScreenState::Pause;
        }
